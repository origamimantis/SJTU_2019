...
std::string defaultImg = "square.png";
std_msgs::Float64 center_x, center_y;
...
cv::VideoCapture vc;
ros::Publisher center_target_x;
ros::Publisher center_target_y;
...
void processImg ( )
{
	...
	center_target_x.publish(center_x);
    center_target_y.publish(center_y);
}	
...
{
  int state = 0;
  ros::init(ac, av, "Camera_with_Track");
  ros::NodeHandle n;

  // create image transporting handle from the node handle
  image_transport::ImageTransport i(n);
  
  image_transport::Subscriber camData = i.subscribe("ardrone/image_raw", 1, updateImg);
  
  center_target_x = n.advertise<std_msgs::Float64>("centerX", 1);
  center_target_y = n.advertise<std_msgs::Float64>("centerY", 1);
  ros::Rate l_r(0.5);
  ...
  if (ac >= 2)
  {
    ...
    if (basImg.empty())
    {
      std::cout << "Error loading image, shape = 'square.png'"<<std::endl;
      basImg = cv::imread(defaultImg, cv::IMREAD_GRAYSCALE);
    }
  }
  else
  {
    std::cout << "No arg given, shape = 'square.png'"<<std::endl;
    basImg = cv::imread(defaultImg, cv::IMREAD_GRAYSCALE);
