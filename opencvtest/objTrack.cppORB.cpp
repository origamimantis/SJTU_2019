...
cv::Mat curImg, modImg, descr;
cv::Mat baseIMG = cv::imread("shape.png");
std::vector< cv::KeyPoint> shapePt;
...
void processImg ( )
{
	...
	cv::cvtColor( curImg, modImg, cv::COLOR_BGR2GRAY);
...
*/
	...
	*/
	...
	////////////////////////////////////
	std::vector< cv::KeyPoint> newPt;
	cv::Mat curDescr;
	detector->detectAndCompute(modImg, cv::noArray(),newPt, curDescr);
	descr.convertTo(descr, 5);
	curDescr.convertTo(curDescr, 5);
	//cv::FlannBasedMatcher matcher;// = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
	cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
    std::vector< std::vector<cv::DMatch> > knn_matches;
    matcher->knnMatch( descr, curDescr, knn_matches, 2 );


	//std::vector<cv::DMatch> matches;

    //matcher.match( descr, curDescr, matches );
    //matcher.match( descr, descr, matches );
	std::cout<< 0 << std::endl;
...
/*
    ...
    //-- Draw matches
	cv::imshow("base" , baseIMG);
	if (good_matches.size() >= 1)
	{
		std::cout<< "trying to draw" << std::endl;
		cv::Mat img_matches;
		cv::Mat img_matchescur;
		img_matches.convertTo(img_matches, 5);
		img_matchescur.convertTo(img_matches, 5);
		cv::drawMatches( baseIMG, shapePt, modImg, newPt, good_matches, img_matches, cv::Scalar::all(-1),
                 cv::Scalar::all(-1), std::vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS );
		cv::drawMatches( baseIMG, shapePt, curImg, newPt, good_matches, img_matchescur, cv::Scalar::all(-1),
                 cv::Scalar::all(-1), std::vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS );
		cv::imshow("yeet" , img_matches);
		cv::imshow("yeet2" , img_matchescur);
	}
	//////////////////////////////
...
int main(int ac, char** av)
{
	...
	detector->detectAndCompute(baseIMG, cv::noArray(),shapePt, descr);
	descr.convertTo(descr, 5);
